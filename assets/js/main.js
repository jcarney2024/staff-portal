    ...
    var Position = {
        ...
        methods: {
            positionAt(element, target, boundary) {
                ...
                let offset = toPx(this.offset === false ? getCssVar("position-offset", element) : this.offset, this.axis === "x" ? "width" : "height", element);
    ...
    var drop = {
        ...
        methods: {
            ...
            position() {
                ...
                const scrollParentOffset = offset(scrollParents(boundary && this.boundaryAlign ? boundary : this.$el)[0]);
                const boundaryOffset = boundary ? offset(boundary) : scrollParentOffset;
                css(this.$el, "maxWidth", "");
                const maxWidth = scrollParentOffset.width - 2 * toPx(getCssVar("position-viewport-offset", this.$el));
                if (this.pos[1] === "justify") {
                    const prop = this.axis === "y" ? "width" : "height";
                    css(this.$el, prop, Math.min((boundary ? boundaryOffset : offset(this.target))[prop], scrollParentOffset[prop] - 2 * toPx(getCssVar("position-viewport-offset", this.$el))));
    ...
    var navbar = {
        ...
        events: [ {
            ...
            handler(_, _ref10) {
                ...
                this._observer = observeResize($el, () => this.transitionTo(offset($el).bottom - offset(this.dropbar).top + toFloat(css($el, "marginBottom")), $el));
            }
        }, {
            name: "beforehide",
            el() {
